...
namespace {
    vector<Pixel> redFilter(const vector<Pixel> &notFiltered, const size_t threshold, const Pixel &filteringPixel) {
        ...
        if (centralPixel.r > maxRedBrightness or centralPixel.r < minRedBrightness) {
            auto newCentralPixel = Pixel(median.r, centralPixel.g, centralPixel.b);
            vector<Pixel> filtered = notFiltered;
            filtered[getIndexInVector(filtered, filteringPixel)] = newCentralPixel;
            return filtered;
        }
        return notFiltered;
    }
    ...
    vector<Pixel> greenFilter(const vector<Pixel> &notFiltered, const size_t threshold, const Pixel &filteringPixel) {
        ...
        if (centralPixel.g > maxGreenBrightness or centralPixel.g < minGreenBrightness) {
    ...
    vector<Pixel> blueFilter(const vector<Pixel> &notFiltered, const size_t threshold, const Pixel &filteringPixel) {
        ...
        if (centralPixel.b > maxBlueBrightness or centralPixel.b < minBlueBrightness) {
...
Pixel RGBPixelMedianFilter::filter(Neighbourhood& notFiltered, size_t filtrationThreshold) {
    ...
    auto greenFiltered = greenFilter(notFiltered.getNeighbouringPixels(), filtrationThreshold,
                                     notFiltered.getCentralPixel());

    auto blueFiltered = blueFilter(notFiltered.getNeighbouringPixels(), filtrationThreshold,
                                   notFiltered.getCentralPixel());
