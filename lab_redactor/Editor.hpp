...
public:
    ...
    explicit Editor(const std::vector<std::string> &_paragraphs) : paragraphs(_paragraphs),
                                                                   backupParagraphs(_paragraphs) {}

    void deleteLines(unsigned long start, unsigned long finish);

    void changeLines(unsigned long start, unsigned long finish,
                     const std::string &newText);

    void insertLines(unsigned long pos, const std::string &additionalText);

    void replaceLines(unsigned long start, unsigned long finish,
