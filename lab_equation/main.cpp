...
TEST(Bisection, Valid) {
    ...
    EXPECT_LE(std::abs(-4.6224989991992 - Solver(std::make_unique<Bisection>()).Calculate(sq2, -6, 0, 0.00000000001)),
              0.00000000001);
    EXPECT_LE(std::abs(0.57000652648718 - Solver(std::make_unique<Bisection>()).Calculate(coub2, -2, 3, 0.00000001)),
              0.00000001);
    EXPECT_LE(std::abs(1.769292354238635 -
                       Solver(std::make_unique<Secant>()).Calculate(coub_cycle, 1, 3, 0.00000000000001, dcoub_cycle)),
              0.00000000000001);
}
...
TEST(Secant, Valid) {
    ...
    EXPECT_LE(std::abs(-4.6224989991992 - Solver(std::make_unique<Secant>()).Calculate(sq2, -6, 0, 0.000000001)),
              0.000000001);
    EXPECT_LE(std::abs(0.57000652648718 - Solver(std::make_unique<Secant>()).Calculate(coub2, -2, 3, 0.000000001)),
              0.000000001);
    EXPECT_LE(std::abs(1.769292354238635 -
                       Solver(std::make_unique<Secant>()).Calculate(coub_cycle, 1, 3, 0.00000000000001, dcoub_cycle)),
              0.00000000000001);
...
TEST(Newton, Valid) {
    ...
    EXPECT_LE(std::abs(-4.6224989991992 - Solver(std::make_unique<Newton>()).Calculate(sq2, -6, -4, 0.0000001, dsq2)),
              0.0000001);
    EXPECT_LE(std::abs(0.57000652648718 - Solver(std::make_unique<Newton>()).Calculate(coub2, 0, 3, 0.0000001, dcoub2)),
              0.0000001);
    EXPECT_LE(std::abs(1.769292354238635 -
                       Solver(std::make_unique<Newton>()).Calculate(coub_cycle, 1, 3, 0.00000000000001, dcoub_cycle)),
              0.00000000000001);
}

TEST(Newton, InValid) {
    EXPECT_TRUE(
            std::isnan(Solver(std::make_unique<Newton>()).Calculate(coub_cycle, -3, 3, 0.000000000001, dcoub_cycle)));
